...
processMIDAS_tmp <- function(pkn, rawMIDASpath, processedMIDASpath, cellLines){
  ...
  for (myCellLine in cellLines){
    ...
    t0control[grep("DV:", colnames(MyMIDAS.new))]<-0 # readouts
    t1control<-t0control
    ...
    fileName<-paste("MIDAS-", myCellLine, ".csv", sep="")
